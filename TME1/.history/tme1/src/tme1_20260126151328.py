...
import os
os.chdir("C:\_PHD\TMEs ML\TME1\\tme1")
...
class Density(object):
    def fit(self,data):
        """ permet d’apprendre l’estimateur sur les données passées en argument
    ...
    def predict(self,data):
        """ permet de prédire pour chaque point de data la densité
...
def load_poi(typepoi,fn=POI_FILENAME):
    ...
    note = np.array([ for v in sorted(poidata[typepoi].items())])
    return data,note
    
### A Faire  ###
def score(data):
    """Renvoie la log-vraisemblance des données (data) de l'estimateur
...
if __name__ == "main":
    ...
    plt.ioff() 
    plt.show()v[1][1]
    # Ajotué les deux dernières lignes sinon le plot disparaît
