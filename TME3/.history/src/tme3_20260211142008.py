...
def perceptron_grad(w,x,y):
    res = y*np.dot(x,w)
    return np.where(res > 0, 0, -y*)  # Si y et x@w ont le même signe alors pas d'erreur, cette donnée ne va pas modifier le poids, si c'est négatif alors erreur, on met moins parce que le gradient est censé être positif
