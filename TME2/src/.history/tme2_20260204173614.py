...
def mse_grad(w,x,y):
    ...
    return 2*np.dot(x.T, no.dot(x,w))
...
def reglog_grad(w,x,y):
    y=y.reshape(-1,1)
    w=w.reshape(-1,1)
    return (-y*x)*np.exp(-y*(x@w))*1/(1+np.exp(-y*np.dot(x,w)))
...
def descente_gradient(datax, datay, f_loss, f_grad, eps, iter):
    ...
    """
    w = np.random.uniform((datax.shape[1], 1))

    list_w = [w]
    list_loss = [np.mean(f_loss(w,datax,datay))]

    for i in range(iter):
        
        w = w - eps*np.mean(f_grad(w, datax, datay))
        loss = np.mean(f_loss(w,datax,datay))
